...
const DiscountManagement = () => {
  ...
  const couponMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(editingCoupon ? 'Data voucher berhasil diperbarui.' : 'Voucher baru berhasil diterbitkan.');
      refetchCoupons();
      setIsModalOpen(false);
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message || 'Gagal menyinkronkan data voucher.');
    }
  });
  ...
  const onConfirmDelete = async () => {
    const t = toast.loading('MENGHAPUS VOUCHER...');
    try {
      await api.delete(`/admin/coupons/${deleteConfig.id}`);
      toast.success(`Voucher ${deleteConfig.code} berhasil dihapus.`);
      refetchCoupons();
      setIsConfirmOpen(false);
    } catch { toast.error('Gagal menghapus data.'); } finally { toast.dismiss(t); }
  };
  ...
  const toggleStatus = async (coupon: any) => {
    try {
      ...
      toast.success(`Status voucher: ${!coupon.is_active ? 'AKTIF' : 'NONAKTIF'}`);
      refetchCoupons();
  ...
  return (
    <AdminLayout>
      <div className="space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 pb-10 border-b border-white/5">
          <div>
            <h1 className="font-display font-black text-4xl uppercase tracking-tighter mb-3 italic">
              ARSENAL <span className="text-accent underline decoration-4 underline-offset-8">PROMOSI</span>
            </h1>
            <p className="text-white/30 text-[10px] font-black uppercase tracking-[0.4em]">Manajemen Voucher Diskon & Kode Promosi</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-8">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/20 mb-3">VOUCHER AKTIF</p>
          ...
          <div className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-8">
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/20 mb-3">TOTAL DIKELOLA</p>
            <div className="flex items-center justify-between">
          ...
          <div className="bg-[#111] border border-white/10 rounded-[2rem] p-8 relative overflow-hidden">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-white/20 mb-3 relative z-10 font-bold">STATUS_SISTEM</p>
            <div className="flex items-center justify-between relative z-10">
              <h4 className="text-accent font-display font-black text-2xl uppercase italic [text-shadow:0_0_20px_rgba(251,133,0,0.4)]">MENYIARKAN</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <div className="col-span-full h-96 flex flex-col items-center justify-center border border-dashed border-white/10 rounded-[3rem] opacity-30">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em]">Belum ada voucher diskon yang diterbitkan.</p>
            </div>
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-center gap-3 mb-6">
                  <span className="px-4 py-1.5 bg-accent text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-xl shadow-2xl shadow-accent/20 italic">
                    {coupon.type === 'percentage' ? `${Number(coupon.value)}% DISKON` : `Rp ${Number(coupon.value).toLocaleString('id-ID')} POTONGAN`}
                  </span>
                  <span className="text-[9px] font-black uppercase tracking-widest text-white/20">Kuota: {coupon.usage_limit || '∞'}</span>
                </div>
                <div className="space-y-3 pt-6 border-t border-white/5">
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                    <span className="text-white/20 flex items-center gap-2 font-bold"><Clock className="w-3.5 h-3.5 text-accent" />BERAKHIR</span>
                    <span className="text-white/50 italic">{coupon.valid_until ? new Date(coupon.valid_until).toLocaleDateString('id-ID') : 'SELAMANYA'}</span>
                  </div>
                  <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest">
                    <span className="text-white/20 flex items-center gap-2 font-bold"><ArrowRight className="w-3.5 h-3.5 text-accent" />MIN_PEMBELIAN</span>
        ...
        <div className="px-10 py-8 border-b border-white/5 bg-[#111] flex items-center justify-between shrink-0 font-body">
           <div>
              <h3 className="font-display font-black text-3xl uppercase tracking-tighter italic text-white">
                {editingCoupon ? 'UBAH DATA' : 'TERBITKAN'} <span className="text-accent underline decoration-4">VOUCHER</span>
              </h3>
              <p className="text-white/20 text-[9px] font-black uppercase tracking-[0.3em] mt-2">Terapkan logika diskon temporal ke dalam katalog inti.</p>
           </div>
           ...
           <form id="couponForm" onSubmit={handleSubmit} className="space-y-10">
              <div className="grid grid-cols-2 gap-8 h-fit">
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 flex items-center gap-2 font-bold"><Tag className="w-4 h-4 text-accent" />Kode Voucher</label>
                    <input type="text" required value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase() })} placeholder="CONTOH: DISKON24" className="w-full h-16 px-8 bg-white/5 border border-white/5 rounded-2xl text-[14px] font-black tracking-widest text-white outline-none focus:border-accent transition-all font-mono uppercase" />
                 </div>
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 flex items-center gap-2 font-bold"><Layers className="w-4 h-4 text-accent" />Protokol Logika</label>
              ...
              <div className="grid grid-cols-2 gap-8 h-fit">
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 font-bold">Nilai Potongan</label>
                 ...
                 <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 font-bold">Kuota Penggunaan</label>
                    <input type="number" value={formData.usage_limit || ''} onChange={(e) => setFormData({ ...formData, usage_limit: e.target.value })} placeholder="∞ TANPA BATAS" className="w-full h-16 px-8 bg-white/5 border border-white/5 rounded-2xl text-[13px] font-black text-white outline-none focus:border-accent placeholder:text-white/10" />
                 </div>
              </div>

              <div className="grid grid-cols-2 gap-8 h-fit">
                 <div className="space-y-3 text-right">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 font-bold">Batas Minimum Belanja</label>
                 ...
                 <div className="space-y-3 text-right">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 font-bold">Maksimum Diskon</label>
              ...
              <div className="grid grid-cols-2 gap-8 h-fit">
                 <div className="space-y-3 text-right">
                    <label className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 font-bold">Waktu Mulai</label>
              ...
              <div className="flex items-start gap-4 p-6 bg-accent/5 border border-accent/10 rounded-[2rem] h-fit">
                 ...
                 <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] leading-relaxed">Dengan menerbitkan voucher ini, Anda mengakui bahwa data akan segera disinkronkan ke seluruh node transaksi aktif di sektor terkait.</p>
              </div>
           </form>
        </div>

        <div className="px-10 py-8 border-t border-white/5 bg-[#111] flex items-center justify-between shrink-0 font-body">
           <button type="button" onClick={() => setIsModalOpen(false)} className="h-16 px-8 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-white/20 border border-white/5 hover:text-white hover:bg-white/5 transition-all">BATALKAN</button>
           <button type="submit" form="couponForm" disabled={couponMutation.isPending} className="h-16 px-12 bg-white text-black font-black uppercase text-[11px] tracking-[0.3em] rounded-2xl hover:bg-accent hover:text-white transition-all shadow-2xl disabled:opacity-50 italic">
              {couponMutation.isPending ? 'MEMPROSES...' : (editingCoupon ? 'SIMPAN PERUBAHAN' : 'TERBITKAN SEKARANG')}
           </button>
        </div>
      </AdminModal>
      ...
      <ConfirmModal
        ...
        title="HAPUS VOUCHER"
        message={`KONFIRMASI PENGHAPUSAN PERMANEN UNTUK "${deleteConfig.code}". TINDAKAN INI AKAN MEMUTUSKAN SELURUH KERANJANG AKTIF YANG MENGGUNAKAN KODE INI.`}
